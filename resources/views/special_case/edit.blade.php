    ...
    <div class="card border-dark">
        ...
        <div class="card-body">
         ...
         <tr>
           ...
           <form action="{{route('special_case.update')}}" method="post">
               {{csrf_field()}}
           <div class="row" style="direction: rtl;">
            <div class="form-group col-md-6">   
                ...
                <select class="form-control" name="subject_id">
                    ...
                    <option  value="{{$subject->id}}" {{ $student_special_case->subject()->first()->name == $subject->name ? "selected":"" }}>{{$subject->name}} </option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="form-group col-md-6">   
                ...
                <select class="form-control" name="special_case">
                    ...
                    <option  value="{{$special_case->value}}" {{ $student_special_case->case_type == $special_case->value ? "selected":"" }}>{{$special_case->value}}</option>
                    @endforeach
                </select>
          </div>
          <input type="text" name="student_id" hidden value="{{$student->id}}">
          <input type="text" name="student_special_case" hidden value="{{$student_special_case->id}}">
